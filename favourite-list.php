...
$query = "SELECT * FROM `favorite_lots`";
...
<html lang="en">
  <head>
    ...
    <title>Список избранных лотов</title>
...
if($resultmestopostavki){
    ...
    mysqli_free_result($resultmestopostavki);
		...
		<th scope="col">Ссылка на лот</th>
					  ...
					  <input type="hidden" name="query" value="SELECT * FROM `lots` WHERE id_lot IN (SELECT id_lot FROM favorite_lots)">
	<div id="savebutton1">
...
$querygg = 'SELECT * FROM `lots` WHERE id_lot="'.$rows[0].'"';
...
<?php 
		$queryggs = 'SELECT * FROM `favorite_lots` WHERE id_lot="'.$rows[0].'"';
