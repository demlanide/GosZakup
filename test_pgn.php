...
<script type="text/javascript" src="//ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js"></script>
    ...
    <link rel="icon" href="https://gz.open-k.com/assets/assets/img/favicon.png">
	  ...
	  <style type="text/css">
		  ...
		  tr, td, th{border: 1px solid gray; display:block;}
		  thead > tr,tbody {
  display: block;
}
.rating-area {
...
?>
    ...
    <tr>
      <th id="sl" scope="col" width="30">ID</th>              
		<th scope="col">Номер на госзакупе</td>       
		<th id="nm" scope="col">Наименование</td>	 
		<th scope="col">Регион</td>                   
		<th scope="col">Сумма</td>      	 	
		<th scope="col">Место поставки</td>       
		<th scope="col">Условия поставки </td>		       
		<th scope="col">Заказчик БИН</td>
		<th scope="col"><select id="value" onchange="show()"><option value="0">Дата публикации</option><option value="11">Январь</option></select></td>
		<th scope="col">Дата начала</td>
		<th scope="col">Дата окончания</td>
		<th scope="col">Способ закупки</td>
		<th scope="col">Ссылка на лот</td>
			</tr>
  </thead>
				  <tbody>
<?php 
...
$page = intval($page);
  ...
  if($page > $total) $page = $total;
	$start = $page * $num - $num;
$querypag = "SELECT * FROM lots LIMIT $start, $num";
...
?>
 ...
 <!--<div class="media align-items-center">
                ...
                <div class="media-body text-truncate">-->
					<tr style="" class="horizontal-scroll-wrapper start_date" data-date="11">
						<td style="white-space: pre-wrap;width:30px;" width="94"><? echo $row[0];?>
						<?php // $fori++; echo $fori; ?></td>
					<td style="white-space: pre-wrap;"><a class="flexbox align-items-center flex-grow gap-items text-truncate" href="#qv-user-detailss<? echo $row[0];?>" data-toggle="quickview"><? echo $row[4];?></a></td>
					<td style="white-space: pre-wrap;"><? echo $row[2];?></td>
					<td style="white-space: pre-wrap;"><? echo $row[6];?></td>
					<td style="white-space: pre-wrap;"><? echo $row[9];?></td>
					<td style="white-space: pre-wrap;">Место поставки</td>
					<td style="white-space: pre-wrap;">Условия поставки</td>
					<td style="white-space: pre-wrap;"><? echo $row[8];?></td>
					<td style="white-space: pre-wrap;"><? echo $row[13];?></td>
					<td style="white-space: pre-wrap;"><? echo $row[14];?></td>
					<td style="white-space: pre-wrap;"><? echo $row[15];?></td>
					<td style="white-space: pre-wrap;"><? echo $row[3];?></td>
					<td style="">Ссылка на лот</td>
						</tr>
                <!--</div>
            </div>-->
...
$page2right = '';
$nextpage = '';	  
if ($page != 1) $pervpage = '<a href= ./pagination.php?page=1><<</a> <a href= ./pagination.php?page='. ($page - 1) .'><</a> ';
// Проверяем нужны ли стрелки вперед
if ($page != $total) $nextpage = ' <a href= /pagination.php?page='. ($page + 1) .'>></a>
                                   <a href= ./pagination.php?page=' .$total. '>>></a>';
...
for(var i=0; i<100; i++) {
  	if(document.getElementsByClassName("start_date")[i].dataset.date==month){
	document.getElementsByClassName("start_date")[i].style.display ="none";
	}
